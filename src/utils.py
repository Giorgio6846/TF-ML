...
def process_coingecko_hourly(prices: list) -> pd.DataFrame:
    """
    Given a list of [timestamp, price] pairs from CoinGecko, return a DataFrame
    with the same feature columns as priceDataframe (but only for price, hour_cos, hour_sin, dayweek_cos, dayweek_sin).
    """
    ...
    # Only keep the columns needed for model input
    return df[["price", "hour_cos", "hour_sin", "dayweek_cos", "dayweek_sin"]]

def priceDataframe(name, training=True):
    if training:
